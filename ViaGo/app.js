...
function App() {
  try {
    ...
    React.useEffect(() => {
      if (['shipments', 'tracking', 'driver'].includes(activeTab)) loadShipments();
    }, [activeTab]);
    ...
    return (
      <div className="min-h-screen bg-gray-50" data-name="app" data-file="app.js">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {activeTab === 'new'       && <ShipmentForm onSubmit={handleCreateShipment} />}
          {activeTab === 'shipments' && <ShipmentList shipments={shipments} loading={loading} onRefresh={loadShipments} />}
          {activeTab === 'tracking'  && <TrackingMap shipments={shipments} />}
          {activeTab === 'driver'    && <DriverPanel shipments={shipments} onUpdateShipment={handleUpdateShipment} />}
          {activeTab === 'reports'   && <ReportsPanel shipments={shipments} />}
          {activeTab === 'fleet'     && <FleetManagement />}
          {activeTab === 'billing'   && <BillingPanel shipments={shipments} />}
        </main>
      </div>
    );
  } catch (error) {
