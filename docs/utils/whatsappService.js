...
const WhatsAppService = {
  ...
  openMaps(address, clientCity) {
    const encoded = encodeURIComponent(this._fullAddress(address, clientCity));
    if (this._isIOS()) return `https://maps.apple.com/?q=${encoded}&dirflg=d`;
    return `https://www.google.com/maps/search/?api=1&query=${encoded}`;
  },

  openRoute(addresses, clientCities) {
    if (!addresses || addresses.length === 0) return '#';
    const full = addresses.map((a, i) => this._fullAddress(a, clientCities && clientCities[i]));
